...
return new class extends Migration
{
        ...
        switch ($driver) {
            ...
            case 'mysql':
                ...
                if (!empty($columns)) {
                    ...
                    if (strpos($currentType, 'blocked') === false) {
    ...
    {
        $blockedCount = DB::table('members')->where('status', 'blocked')->count();

        if ($blockedCount > 0) {
            throw new \Exception("Cannot rollback: {$blockedCount} members with 'blocked' status exist. Please resolve these first.");
        }
